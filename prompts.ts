...
export const SERVER_PREDICTION_PROMPTS: ServerPredictionPrompt[] = [
  {
    id: 'custom',
    template: ({input, customPrompt}) => {
      assertString(customPrompt)

      // Normalize the prompt:
      // - Remove trailing period and add colon.
      // - Add a colon if trailing character is a word.
      const customPromptWithDelimiter = customPrompt
        .trim()
        .replace(/\.$/, ':')
        .replace(/(\w)$/, '$1:')

      if (input) {
        return `Use the following text in triple quotes as context for your response:
"""
${input}
"""
${customPromptWithDelimiter}`
      }

      return customPromptWithDelimiter
    },
  },
  {
    id: 'continuation',
